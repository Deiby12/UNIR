...
class Parser:
    ...
    def parse_cond(self):
        """
        parse_cond := parse_xor (('→') parse_xor)*
...
# ==========================================================
# = 2) APLICACIÓN tkinter/ttk CON TECLADO LÓGICO           =
# ==========================================================

class TruthTableGenerator:
    def __init__(self, root):
        self.root = root
        self.root.title("Generador de Tablas de Verdad (con Parser y Negaciones)")
    ...
    def generate_table(self):
        ...
        Genera la tabla de verdad al pulsar el botón:
          ...
          4) (NUEVO) Añade columnas para las negaciones de cada variable
          5) Evalúa cada combinación y la muestra en la tabla
          6) Determina si es tautología, contradicción o contingencia
        """
        ...
        # 3) Extraer subexpresiones entre paréntesis
        ...
        # 4) (NUEVO) Añadir columnas de negación para cada variable
        #    Por ejemplo, si las variables son [p, q, r], añadimos ~p, ~q, ~r.
        neg_list = []
        for var in variables:
            neg_expr = f"~{var}"
            try:
                neg_ast = parse(neg_expr)
                subexpr_asts[neg_expr] = neg_ast
                neg_list.append(neg_expr)
            except ValueError:
                pass  # Por si acaso, pero normalmente no fallará.
        
        # Añadimos la expresión principal como última "subexpresión"
        subexpr_list = neg_list + subexpr_list  # primero negaciones, luego subexprs
        subexpr_list.append(expression)
        subexpr_asts[expression] = main_ast
        
        # Quitamos duplicados (por si ~s ya aparecía en la fórmula entre paréntesis)
        subexpr_list = list(dict.fromkeys(subexpr_list))  # mantiene orden, quita repes
        ...
        # primero las variables, luego negaciones + subexpresiones, y la principal al final
        ...
        for combination in truth_combinations:
            ...
            # Luego evaluamos cada subexpresión (incluyendo negaciones y la principal)
            subexpr_results = []
            for subexpr in subexpr_list:
            ...
            # El resultado de la expresión principal es el último subexpr
            if subexpr_results and subexpr_results[-1] is not None:
                final_results.append(subexpr_results[-1])
        
        # 6) Clasificar la expresión completa (última columna)
        if final_results:
